...
/*
void quicksortByDim(QList<Geometry*> objects, int dim, int start, int end) {
    if (objects.length() > 1) {
        int pivot = (end-start)/2;
        int left = 0;
        int right = end;
        while (objects[left]->bounding_box->center[dim]
               <= objects[right]->bounding_box->center[dim]) {
            while (objects[left]->bounding_box->center[dim]
                   < objects[pivot]->bounding_box->center[dim]) {
                left += 1;
            }
            while (objects[right]->bounding_box->center[dim]
                   > objects[pivot]->bounding_box->center[dim]) {
                right -= 1;
            }
            if (objects[left]->bounding_box->center[dim]
                    <=  objects[right]->bounding_box->center[dim]) {
        ...
        quicksortByDim(objects, dim, start, right);
        quicksortByDim(objects, dim, left, end);
    }
}
*/
